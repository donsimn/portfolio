  ...
  <div class="col-span-12">
    <Reveal>
      ...
      <p class="text-lg md:text-xl opacity-60 max-w-2xl">
        Writeups and reflections on cybersecurity, AI, and learning by breaking
        things.
      </p>
    </Reveal>
  </div>
    ...
    <div class="space-y-0">
      {#each posts as post, index}
        <Reveal delay={index * 50}>
          <article
            class="blog-item py-8 md:py-12 border-t-2 border-black dark:border-white first:border-t-0"
          >
            <a href="/insights/{post.slug}" class="block group">
                ...
                {#if post.tags && post.tags.length > 0}
                  ...
                  <div class="flex flex-wrap gap-2">
                    {#each post.tags as tag}
                      <span
                        class="font-mono text-xs uppercase tracking-wider opacity-40"
                      >
                        {tag}
                      </span>
                    {/each}
              ...
              <!-- Description -->
              <p class="text-base md:text-lg opacity-60 max-w-3xl">
                {post.description}
              </p>
